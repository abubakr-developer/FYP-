...
import { GraduationCap, AlertCircle, Info, ArrowLeft, Mail } from "lucide-react";
...
export default function StudentForgotPassword() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/auth/forgetPassword`, {
        method: "POST",
            ...
            {emailSent && (
              <div className="space-y-4 pt-4">
                <Button
                  variant="default"
                  className="w-full"
                  onClick={() => navigate("/student-reset-password", { state: { email } })}
                >
